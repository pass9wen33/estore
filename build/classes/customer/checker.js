...
xmlHttp.responseText);
	...
	xmlHttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	              xmlHttp.send("name=" + encodeURIComponent(_node.id) + "&value="+ 
	              encodeURIComponent(_node.value));	//发送包含用户输入信息的请求体
        	 	}
    		}
    		//显示服务器反馈信息
    		...
    		this.createXmlHttp = function() {
        		...
        		if (window.XMLHttpRequest) {
           	  xmlHttp = new XMLHttpRequest(); //FireFox、Opera等支持的创建方式
        		} else {
    		...
    		this.checkPassword = function() {
        		...
        		if (p1 != "" && p2 != "") {
        	   		if (p1 != p2) {
              	this.showInfo("password2" + Checker._infoDivSuffix, "0两次输入密码不一致!");
        	   		} else{
